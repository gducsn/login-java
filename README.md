...
Infine il metodo che ci permette di recuperare le informazioni dal db.
...
public class UserDAO {
	...
	private String queryString = "SELECT * FROM login.user WHERE 
...
La prima parte della classe consiste nella creazione di tre stringhe che 
ci serviranno per definire l’accesso al database e la query.
...
Il metodo getConnection e le due stringhe ‘CONTEXT’ e ‘DATASOURCE’ ci 
servono per stabilire la connessione con il database. Questo approccio è 
molto comune, ho spiegato il suo funzionamento 
[qui](https://github.com/gducsn/db-java-crud).
...
Inizialmente il cursore è puntato prima della prima riga. Possiamo usare 
quindi il metodo .next() che ci offre varie opportunità, come iterare 
sugli oggetti presenti in tabella. In questo caso non ci serve iterare, ma 
sfruttare il metodo next() per ritornare un valore boolean. Infatti, se 
non sono presenti righe ritorna FALSE, altrimenti TRUE.

```java
return rs.next();
```

Scrivendo in questo modo significa “ritorna TRUE se è presente una riga”.
...
Quindi la query è molto importate:
...
appunto, raccogliamo il valore emesso dall’utente.
...
altrimenti sarà gestito l’errato login da parte dell’utente.
